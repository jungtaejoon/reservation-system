...
<body>
    <div id="container">
        ...
        <div class="ct">
            <div class="ct_wrap">
            	<script id="product_info_template" type="text/x-handlebars-template">
                <div class="top_title">
                    <a href="#" onClick="history.back();" class="btn_back" title="이전 화면으로 이동"> <i class="fn fn-backward1"></i> </a>
                    <h2><span class="title">{{info.name}}</span></h2>
                </div>
                <div class="group_visual">
                    <div class="container_visual" style="width: 414px;">
                        <ul class="visual_img">
                            <li class="item" style="width: 414px;"> <img alt="" class="img_thumb" src="{{image.saveFileName}}"> <span class="img_bg"></span>
                                <div class="preview_txt">
                                    <h2 class="preview_txt_tit">{{info.name}}</h2> <em class="preview_txt_dsc">{{info.description}}</em>
								</div>
                            </li>
                        </ul>
                    </div>
                ...
                <div class="section_store_details">
                    <div class="store_details">
                        <h3 class="in_tit">{{info.name}}</h3>
                        <p class="dsc">
                            	장소 : {{info.placeName}}<br> 기간 : {{info.displayStart}} ~ {{info.displayEnd}}
                        </p>
                        <h3 class="in_tit">관람시간</h3>
                        <p class="dsc">
                            {{info.observationTime}}
                        </p>
				...
				<script id="price_info_template" type="text/x-handlebars-template">
					{{#prices}}
						{{priceTypeString}}: {{price}} <br>
					{{/prices}}
				</script>
                <div class="section_booking_ticket">
                    ...
                    <!-- [D] 금액이 0 이상이면 individual_price에 on_color 추가 -->
                    	<script id="price_control_template" type="text/x-handlebars-template">
						{{#prices as |priceOb index|}}
                        <div class="qty" data-index="{{index}}">
                            ...
                            <div class="qty_info_icon"> 
                            	<strong class="product_amount"> <span>{{priceTypeString}}</span> </strong> 
                            	<strong class="product_price"> <span class="price">{{discountPrice}}</span> <span class="price_type">원</span> </strong> 
                            	<em class="product_dsc">{{discountPrice}} {{discountRateString}}</em>
                            </div>
                        </div>
						{{/prices}}
						</script>
    ...
    <script src="/resources/scripts/lib/component.min.js"></script>
    <script src="/resources/scripts/reserve.js"></script>
</body>
