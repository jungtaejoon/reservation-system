...
<head>
    ...
    <!-- Product   -->
    <script id="reserve-image-templ" type="text/x-handlebars-template">
        {{#each files}}
            {{#if @first}}
                <li class="item" style="width: 414px;"> 
                    <img alt="" class="img_thumb" src="/files/{{id}}"> <span class="img_bg"></span>
                <div class="preview_txt">
                    <h2 class="preview_txt_tit">{{lookup ../this "name"}}</h2> 
                    <em class="preview_txt_dsc">₩{{lookup ../this "minCost"}} ~ </em>
                    <em class="preview_txt_dsc">{{lookup ../this "salesStart"}}~{{lookup ../this "salesEnd"}}, 잔여티켓 {{lookup ../this "ticketCount"}}매</em> </div>
            </li>
            {{/if}}
        {{/each}}
    </script>
    ...
    <!-- Store Detail  -->
    <script id="sotre-details-templ" type="text/x-handlebars-template">
        <div class="store_details">
            <h3 class="in_tit">{{name}}</h3>
            <p class="dsc">
                장소 : {{placeStreet}}({{placeLot}})<br> 기간 : {{salesStart}}~{{salesEnd}}
            </p>
            <h3 class="in_tit">관람시간</h3>
            <p class="dsc">
                {{observationTime}}(입장마감 {{deadLineTime}})
            </p>
    ...
    <!-- Counter  -->
    <script id="count-control-templ" type="text/x-handlebars-template">
        {{#prices}}
            <div class="qty" data-pricetype="{{priceType}}">
                <div class="count_control">
                 ...
                 <div class="qty_info_icon"> <strong class="product_amount"> <span>성인</span> </strong> <strong class="product_price"> <span class="price">{{price}}</span> <span class="price_type">원</span> </strong> <em class="product_dsc">{{discountPrice}}원 ({{discountRate}}% 할인가)</em> </div> 
            </div>
        {{/prices}}
    </script>
    ...
    <script id="reserve-form-templ" type="text/x-handlebars-template">
        <form class="form_horizontal">
            ...
            <div class="inline_form last"> <label class="label" for="message">예매내용</label>
                <div class="inline_control">
                    <p class="inline_txt selected">{{salesStart}}~{{salesEnd}}, 총 0매</p>
                </div>
            </div>
        </form>
    </script>
            ...
            <div class="ct_wrap">
                ...
                <div class="group_visual">
                    <div class="container_visual" style="width: 414px;">
                        <ul class="visual_img">
                            <!-- [Handlebars] image list  -->
                        </ul>
                    </div>
                </div>
                <div class="section_store_details">
                    <!-- [Handlebars] store-details-info  -->
                </div>
                <div class="section_booking_ticket">
                    <div class="ticket_body">
                        <!-- [Handlebars] count-control-templ -->
                    </div>
                </div>
