...
public class ProductSqls {
	...
	public static final String GET_LIST_BY_CATEGORY = "SELECT p.id, p.name, p.description, di.place_name, pi.file_id" + 
											...
											" LIMIT :page, 10";
	public static final String GET_DETAIL = "SELECT p.id, p.name, p.description, p.sales_flag, p.event, pd.content, di.observation_time, di.display_start," +
											" di.display_end, di.place_name, di.place_lot, di.place_street, di.tel, di.homepage, di.email " + 
											" FROM product p" + 
